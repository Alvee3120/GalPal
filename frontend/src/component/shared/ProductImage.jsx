...
// `bleed` fills the whole box edge to edge (cropping to fit) with no padding, for a card whose photo spans its full width.
// `tight` uses minimal padding (thumbnails in compact cards); the default suits the large product tile.
export default function ProductImage({ src, alt, tight = false, bleed = false }) {
  ...
  return (
    <Image
      ...
      className={`product-card__img ${bleed ? "object-cover" : `object-contain ${tight ? "p-1" : "p-3 sm:p-5"}`}`}
    />
  );
}
