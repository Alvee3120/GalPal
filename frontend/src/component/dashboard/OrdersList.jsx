...
export default function OrdersList({ initialOrders, initialCount, currencySymbol, basePath = "/dashboard/customer/orders" }) {
  const router = useRouter();
  const [orders, setOrders] = useState(initialOrders);
  const [count, setCount] = useState(initialCount ?? initialOrders.length);
  ...
  async function changePage(nextPage) {
    if (nextPage === page || pageLoading) return;
    setPageLoading(true);
    try {
      const res = await fetch(`/api/orders?page=${nextPage}&page_size=${PAGE_SIZE}`, { cache: "no-store" });
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <DashboardPagination page={page} totalPages={totalPages} onPageChange={changePage} disabled={pageLoading} />
    </div>
  );
}
